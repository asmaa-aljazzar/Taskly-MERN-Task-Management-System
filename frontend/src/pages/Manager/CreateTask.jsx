	...
	useEffect(() => {
		(async () => {
			try {
				...
				// Get team members (NOT including manager)
				let teamMembers = team.members ?? [];
				
				// If members are IDs (strings), we need to fetch their details
				if (teamMembers.length > 0 && typeof teamMembers[0] === 'string') {
					...
					teamMembers = memberDetails.filter(m => m !== null);
				}
				
				// Filter out the manager - only show employees
				const filteredMembers = teamMembers.filter(member => {
					// If we have role information, filter out managers
					if (member.role) {
						return member.role !== 'manager' && member.role !== 'hr';
					}
					// Also filter out by comparing with managerId if available
					if (team.managerId) {
						...
						<div>
							...
							{members.length === 0 && (
								<p className="mt-1 text-xs text-amber-600">
									No employees available in this team. Add employees to the team first.
								</p>
							)}
